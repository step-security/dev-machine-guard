...
- **Self-updating binary on every platform, Windows included**: a scheduler-fired run now keeps its own executable current instead of relying on a loader tick. The binary asks the `latest-binary` endpoint for the release its tenant should run, verifies the checksum's Ed25519 SSHSIG natively against the pinned release key, verifies the download's sha256, and swaps itself in place — the new version takes effect on the next scheduled fire, so there is no re-exec. On Windows it also updates the GUI launcher (`stepsecurity-dev-machine-guard-task.exe`) that the scheduled task actually invokes, because an agent-only update would leave the pair on different releases; both artifacts' signatures are verified before either is downloaded. Windows cannot overwrite or unlink a mapped executable image, so the live `.exe` is renamed aside and the new one takes its path, with the leftover swept on a later run and the original restored if the install fails. Strictly opt-in via the `auto_update` config key the auto-loader writes at install; version-pinned installs and manual runs never set it and can never drift off their pin. An update never blocks a scan, and a release below the first self-updating version is refused — it would strand a binary-scheduled install with no update path. Kill switch: `STEPSEC_DISABLE_SELF_UPDATE=1`.
