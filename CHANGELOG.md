...
- **macOS hostname no longer flips with the network.** The agent reported `kern.hostname`, which macOS rewrites from DHCP or reverse DNS whenever `HostName` is unset, so a Mac on a VPN resolving through AWS showed up as `ip-…ec2.internal` on some scans and its real name on others. It now reports `scutil --get HostName` when set, else `LocalHostName`, falling back to the kernel hostname. Existing Macs will show a new name once.
